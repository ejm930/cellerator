...
interpret::usage="interpret[] converts a list of reactions into differential equations of the form {{ode for variable 1, ode for variable 2, ...}, {variable 1, variable 2, ...}}.\nThe variable Global`t is used for the time argument.\nA reaction has the format {arrow-form, rateConstants}.\nSupported arrow-forms are\n"<>"Mass Action: A\[ShortRightArrow]B, \!\(\*OverscriptBox[\(A \[ShortRightArrow] B\), \(C\)]\), A\[RightArrowLeftArrow]B, \!\(\*OverscriptBox[\(A \[RightArrowLeftArrow] B\), \(En\)]\), \!\(\*OverscriptBox[\(A \[Equilibrium] B\), \(En\)]\), \!\(\*UnderoverscriptBox[\(A \[RightArrowLeftArrow] B\), \(REn\), \(FEn\)]\)"<>"\nMichaelis Menten: A\[DoubleLongRightArrow]B, \!\(\*OverscriptBox[\(A\[DoubleLongRightArrow]B\), \(En\)]\), A\[DoubleLongLeftRightArrow]B, \!\(\*UnderoverscriptBox[\(A\[DoubleLongLeftRightArrow]B\), \(REn\), \(FEn\)]\)"<>" \nHill (catalytic): \!\(\*OverscriptBox[\(\(\\\ \)\(A \[RightTeeArrow] B\)\), \(En\)]\)\nRegulatory: A\[RightTeeArrow]B, (Hill[..], GRN[..\!\(\*SuperscriptBox[\(]\), \(*\)]\), NHCA[..\!\(\*SuperscriptBox[\(]\), \(*\)]\),S-System[..\!\(\*SuperscriptBox[\(]\), \(*\)]\))"<>" \n\!\(\*SuperscriptBox[\(MWC\), \(*\)]\):\!\(\*UnderoverscriptBox[\(S\[DoubleLongRightArrow]P\), \(A\), \(En\)]\)  \n\!\(\*SuperscriptBox[\(GMWC\), \(*\)]\):\!\(\*UnderoverscriptBox[\({\*SubscriptBox[\(S\), \(1\)], \*SubscriptBox[\(S\), \(2\)], \[Ellipsis]}\[DoubleLongRightArrow]{\*SubscriptBox[\(P\), \(1\)], \*SubscriptBox[\(P\), \(2\)], \[Ellipsis]}\), \({{\*SubscriptBox[\(A\), \(1\)], \[Ellipsis]}, {\*SubscriptBox[\(In\), \(1\)], \[Ellipsis]}, \\\ {\*SubscriptBox[\(CI\), \(1\)], \[Ellipsis]}}\), \(En\)]\) "<>"\n\n\!\(\*SuperscriptBox[\(\"\<<>\>\"\), \(*\)]\)in development.";
...
eq4="\!\(\*SuperscriptBox[\(B\), \(\[Prime]\),\nMultilineFunction->None]\)[t]\[Equal]\!\(\*FractionBox[\(v \(\*UnderscriptBox[\(\[Product]\), \(i\)]\*SuperscriptBox[\((1 + SubscriptBox[T, i] \[CapitalTheta][\*SubscriptBox[\(T\), \(i\)]] \*SuperscriptBox[\(\(\*SubscriptBox[\(A\), \(i\)]\)[t]\), SubscriptBox[\(n\), \(i\)]])\), \(m\)]\)\), \(k\\\ \(\*UnderscriptBox[\(\[Product]\), \(i\)]\*SuperscriptBox[\((1 + SubscriptBox[T, i] \[CapitalTheta][\(-\*SubscriptBox[\(T\), \(i\)]\)] \*SuperscriptBox[\(\(\*SubscriptBox[\(A\), \(i\)]\)[t]\), SubscriptBox[\(n\), \(i\)]])\), \(m\)]\) + \*UnderscriptBox[\(\[Product]\), \(i\)]\*SuperscriptBox[\((1 + SubscriptBox[T, i] \[CapitalTheta][\*SubscriptBox[\(T\), \(i\)]] \*SuperscriptBox[\(\(\*SubscriptBox[\(A\), \(i\)]\)[t]\), SubscriptBox[\(n\), \(i\)]])\), \(m\)]\)]\)";
...
MyGraphicsArray[x___]:=If[$VersionNumber<6, GraphicsArray[x], GraphicsGrid[x]]; 
...
expandArrows[{Overscript[RightTeeArrow[X_,Y_ ],En_],hill[v_:1,n_:1,K_:1,a_:0,T_:1]}]:= 
Module[{reaction},
reaction={Overscript[RightTeeArrow[X,Y],En],hill[v,n,K,a,T]};
If[dbgArrows,Print["expandArrows:", reaction]];
Return[{reaction}];
];


parseArrowForm[{Overscript[RightTeeArrow[Plus[X_,X1__],Y_ ],En_],hill[v_:1,n_:1,K_:1,r_:0,m_:1]}]:= 
Module[{odes,rateLaw, vars,x},
reaction={Overscript[RightTeeArrow[Plus[X],Y],En],hill[v,n,K,r,m]};
If[dbgArrows,Print["parseArrowForm:", reaction]];
vars={X,X1}//Flatten;
odes = parseArrowForm[{Overscript[RightTeeArrow[vars,Y],En],hill[v,n,K,r,m]}];
Return[odes];
];

(* this module sig. revised v .0.81 *)
parseArrowForm[{Overscript[RightTeeArrow[X_,Y_ ],En_],hill[v_:1,n_:1,k_:1,r_:0,m_:1]}]:= 
Module[{odes,rateLaw, vars,x,nvars, V, K, nn, rr, multiplier, enzyme},
reaction={Overscript[RightTeeArrow[X,Y],En],hill[v,n,k,r,m]};
If[dbgArrows,Print["parseArrowForm:", reaction]];
...
rateLaw=enzyme*multiplier*hillFunction[x,K, nn,1,0];
rateLaw=rateLaw/.{Global`\[EmptySet][Global`t]-> 0};
...
expandArrows[{RightTeeArrow[X_,Y_ ],hill[v_:1,n_:1,K_:1,r_:0,m_:1]}]:= 
Module[{reaction},
reaction={RightTeeArrow[X,Y],hill[v,n,K,r,m]};
If[dbgArrows,Print["expandArrows:", reaction]];
Return[{reaction}];
];
expandArrows[{RightTeeArrow[Plus[X_,X1__],Y_ ],hill[v_:1,n_:1,K_:1,r_:0,m_:1]}]:= 
Module[{reaction,vars},
vars={X,X1}//Flatten;
reaction={RightTeeArrow[vars,Y],hill[v,n,K,r,m]};
If[dbgArrows,Print["expandArrows:", reaction]];
Return[{reaction}];
];
...
parseArrowForm[{RightTeeArrow[Plus[X_,X1__],Y_ ],hill[v_:1,n_:1,K_:1,r_:0,m_:1]}]:= 
Module[{odes,rateLaw, vars,x, reaction},
vars={X,X1}//Flatten;
odes = parseArrowForm[{RightTeeArrow[vars,Y],hill[v,n,K,r,m]}];
Return[odes];
];

parseArrowForm[{RightTeeArrow[X_,Y_ ],hill[v_:1,n_:1,k_:1,r_:0,mult_:1]}]:= 
Module[{odes,rateLaw, vars,x,nvars,V, K,nn,rr,multiplier, reaction, dummy},
reaction={RightTeeArrow[X,Y],hill[v,n,k,r,mult]};
If[dbgArrows,Print["parseArrowForm:", reaction]];
...
n=Check[NDSolve[eqs, vars, {time, tstart, tend}, nopt],
(Print["Unable to continue simulation."];Abort[])]; 
];
...
gridPlot[sys_, vars_,{tstart_, tend_}, opt___?OptionQ] := Module[{p,v, colors, nv,ga,nc, rem, styles},
If[vars===All,
v=First/@Flatten[sys];,
v=vars;
]; 
...
styles = Flatten[{"styles"/.{opt}/.Options[gridPlot]}];
colors = Append[styles,#]&/@colors; 
...
Return[Show[MyGraphicsArray[ga], DisplayFunction-> $DisplayFunction, optsShow]]];
];
...
Style["MathSBML", FontFamily-> "Ubuntu,Arial", FontSize-> 12, FontWeight-> Bold],
Style[" version "<>MathSBML`$MathSBML$Version<>"\n", FontFamily-> "Ubuntu,Arial", FontSize-> 12],
