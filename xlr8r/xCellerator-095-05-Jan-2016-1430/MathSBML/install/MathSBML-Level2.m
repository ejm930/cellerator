...
debugPrint[dbg,"getReaction: Preparing to call checkKineticLaw."]; 
Block[{re,p,f, locals,myrule,ok},
myrule[x_,y_]:=x; 
re=stringer/@decontextify/@First/@reactants;
debugPrint[dbg, "getReaction: re:", re]; 
p=stringer/@decontextify/@First/@products;
debugPrint[dbg, "getReaction: p: ", p]; 
f=StringReplace[formula,{"[t]"-> "",id<>"`"-> "",$SBML$Context-> ""}];
debugPrint[dbg,"getReaction:  f: ", f]; 
locals=decontextify/@ToString/@((ToExpression/@localParameters)/.{Rule->myrule}); 
debugPrint[dbg, "getReaction: locals: ", locals]; 
debugPrint[dbg,"getReaction: calling checkKineticLaw."]; ok=checkKineticLaw[
