...
(* Load xlr8 r if it is not already in memory *)
...
xlr8r2SBMLVersion="0.4 (24 April 2011)";
...
(* ::Input:: *)
convertToSBML[network_,initialConditions_, rates_, modelid_, modelname_, compartmentname_]:= Module[{debug=False,low,addNextReaction,mybind,nosubscript,kinlaw, handleMassAction,handleMM,handleHill,handleGRN,handleSSystem,handleNHCA,handleGMWC,species,sids,svalues,rids,rvalues, noEmptySet, notes, dPrint},

dPrint[x___]:= If[debug, Print["convertToSBML:", x]]; 
...
kl=ToExpression[kl]/.{Global`EmptySet-> 1} ; (* added EmptySet 0.4 *)
...
dPrint["handleMassAction: ", reaction]; 
...
dPrint["The kinetic law for this reaction is: ",kLaw]; (* Print *)
(* rev 0.04 *)
react=First[reaction]/.{RightTeeArrow-> ShortRightArrow};
...
(* changed so reactant is emptyset v 0.04 *)
(* GRN[] -> { } *)
...
(* Editing the current reaction to fit the SBML spec
 This will turn the xCellerator GRN arrow into the SBML accepted Rule arrow *)react=First[reaction]/.{RightTeeArrow->Rule};

dPrint["react: ", react];
dPrint["prod: ", {product}]; 
dPrint["modi: ", {modi}]; 
dPrint["reactants: ", {reactant}]; 
dPrint["klaw: ", kLaw]; 

(* Adding the current reaction to the model *)
MathSBML`addReaction[react, Global`kineticLaw->kLaw, Global`modifiers->{modi},Global`products->{product},Global`reactants->{Global`EmptySet} ];
...
dPrint["addNextReaction: reaction: ", reaction]; 
...
low=low/.{Global`\[EmptySet]-> Global`EmptySet};
...
rids=rids/.{\[EmptySet]-> Global`EmptySet};
...
sids=sids/.{\[EmptySet]-> Global`EmptySet};

svalues=Map[Last, initialConditions];

newspecies=Complement[species,sids];

MathSBML`addSpecies[Global`id-> Global`EmptySet, Global`initialAmount-> 0]; 
