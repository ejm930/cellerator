...
ExpandReaction[{A_\[ShortRightArrow] B_, x___}]:={{A-> B , x}};
...
SSAFit[sim_,tmax_]:=Module[{vars,nvars,ndata,tfin,vfin,extract,data,ldata,newdata,funcs,fit},
vars=First/@sim;
nvars=Length[vars];
data=sim/.Rule->List;
ldata=Flatten[Rest/@data,1];
For[j=1,j<=nvars,j++,extract=ldata[[j]];ndata=Length[extract];tfin=extract[[-1,1]];vfin=extract[[-1,2]];For[i=1,i<=ndata-1,i++,extract[[-i,2]]=extract[[-(i+1),2]]];
If[tfin<tmax,AppendTo[extract,{tmax,vfin}],Null];
ldata[[j]]=extract];
newdata=MapThread[Rule,{vars,ldata}];
funcs=Interpolation[#/.newdata,InterpolationOrder->0]&/@vars;
fit=MapThread[Rule,{vars,funcs}]]     


SSAMean[sims_,tmax_,opt___?OptionQ]:=Module[{vars,fits,domains,tstart,tend,dt,tvals,i,timespan,npoints,values,sums,t,vals,functionate,ffits,nvars,valsums,valsumsq,\[Mu],means,\[Sigma],sigmas,nsims,isims,times,plus1sigma,plus2sigma,plus3sigma,minus1sigma,minus2sigma,minus3sigma,\[Sigma]p1,\[Sigma]p2,\[Sigma]p3,\[Sigma]m1,\[Sigma]m2,\[Sigma]m3,Interp,dPrint,dbg,Rates,rates,results},dbg="Debug"/.{opt}/.{"Debug"->False};
Rates="Rates"/.{opt}/.{"Rates"->False};
dPrint[x___]:=If[dbg,Print["SSAMean: ",x]];
functionate[f_,val_]:=f[val];
npoints="Points"/.{opt}/.{"Points"->100};
(* *)
dPrint[npoints,"  Points"];
(* *)
timespan="TimeSpan"/.{opt}/.{"TimeSpan"->Automatic};
nsims=Length[sims];
(* *)
dPrint["Simulation End  =  ",tmax];
Print[npoints,"  Points,  ",nsims,"  Trajectories",",  Simulation End  =  ",tmax];
(* *)
...
fits=sims;,Print["Fitting data. Interpolation Level = 0."];
fits=sims;
For[isims=1,isims<=nsims,isims++,fits[[isims]]=SSAFitCor[sims[[isims]],tmax]]];
(*fits=SSAFitCor/@sims];*)
dPrint["fits: ",fits];
fits=(vars/.#)&/@fits;
(*Print["fits: ",fits];*)
...
(* *)
dPrint["tstart = ",tstart,", tend = ",tend,", dt = ",dt];
(* *)
...
(* *)
dPrint["time t = ",t,":   ",valsums,"   ",valsumsq,"   ",\[Mu],"   ",\[Sigma]];
(* *)
\[Sigma]p1={t,#}&/@(\[Mu]+\[Sigma]);
